...
def extendLine(point1, point2, distance):
    """
    ...
    Returns:
    Tuple of tuples representing the coordinates of the expanded line segment
    """
    p1 = np.array(point1)
    p2 = np.array(point2)
...
def get_angle(p1, p2, p3, degree = True): 
    """ 
...
def centerline(
    ...
    while True:
        ...
        lineOrder = sortedCentroidToLine(polygon, model.cluster_centers_)

        if lineOrder:
            coordinates = shapely.get_coordinates(lineOrder)

            if not lineOrder.is_simple:
            ...
            if found: # or n_clusters >= k_max:
                break
            else: 
                n_clusters += 1
                lineK_Order = lineOrder
        else:
            n_clusters += 1
            lineK_Order = lineOrder
